...
@section('title', 'Tambah Transaksi - Manajemen Laundry')

@section('content')
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3">Tambah Transaksi</h1>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.1" class="form-control @error('berat') is-invalid @enderror" 
                                       id="berat" name="berat" value="{{ old('berat') }}" min="0.1" required>
                                @error('berat')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" 
                                    ...
                                    <option value="Baru Masuk" {{ old('status') == 'Baru Masuk' ? 'selected' : '' }}>Baru Masuk</option>
                                    <option value="Dalam Proses" {{ old('status') == 'Dalam Proses' ? 'selected' : '' }}>Dalam Proses</option>
                                    <option value="Selesai" {{ old('status') == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                                    <option value="Sudah Diambil" {{ old('status') == 'Sudah Diambil' ? 'selected' : '' }}>Sudah Diambil</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (layananSelect.value && beratInput.value) {
        calculateTotal();
    }
});
</script>
